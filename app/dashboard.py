...
def _nav_chats(email: str) -> str:
    return f"""
  <div style="display:flex; justify-content:space-between; align-items:flex-start;">
    <div>
      <a href="/dashboard" style="color:{GRAY}; font-size:12px;">← Panel principal</a>
      <h1 style="margin-top:6px;">Conversaciones — Aurora</h1>
    </div>
    {f'<div style="font-size:12px; color:{GRAY};">{email} · <a href="/logout" style="color:{GRAY};">cerrar sesión</a></div>' if email else ""}
  </div>"""
...
def _fila_chat(conv: Conversacion) -> str:
    ...
    return f"""
    <a class="fila-chat" href="/dashboard/chats/{conv.telefono}">
...
def render_lista_chats(chats: list[Conversacion], email: str = "", busqueda: str = "") -> str:
    filas = "".join(_fila_chat(c) for c in chats)
    if not filas:
        filas = '<p class="fila-azul-vacio">Todavía no hay conversaciones registradas.</p>'
...
<style>
  ...
  form.buscar {{ margin: 20px 0; }}
  form.buscar input {{
    width: 100%; box-sizing: border-box; padding: 9px 12px; border: 1px solid {MUTED_GRID};
    border-radius: 8px; font-size: 14px;
  }}
  .fila-chat {{
    display: block; border: 1px solid {MUTED_GRID}; border-radius: 10px; padding: 12px 14px;
    margin-bottom: 10px; text-decoration: none; color: inherit;
  }}
  .fila-chat:hover {{ border-color: {TEAL}; }}
  .fila-chat-encabezado {{ display: flex; justify-content: space-between; align-items: baseline; }}
  .fila-chat-titulo {{ font-size: 14px; font-weight: 600; color: {NAVY}; }}
  .fila-chat-sub {{ font-weight: 400; color: {GRAY}; font-size: 12px; }}
  .fila-chat-hora {{ font-size: 11px; color: {GRAY}; white-space: nowrap; }}
  .fila-chat-extracto {{ font-size: 13px; color: {GRAY}; margin: 4px 0 8px; }}
